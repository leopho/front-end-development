...
(function (global) {
  ...
  var switchMenuToActive = function (pageName) {
    // Remove 'active' from button
    var activeMenuItem = document.querySelector("#collapsable-nav li.active");
    var classes;

    if (activeMenuItem != null) {
      classes = activeMenuItem.className;
      classes = classes.replace(new RegExp("active", "g"), "");
      activeMenuItem.className = classes;
    }
  ...
  dc.loadAbout = function() {
    ...
    $ajaxUtils.sendGetRequest(aboutHtmlUrl, function(aboutHtmlContent) {
      // Switch CSS class active to menu button
      switchMenuToActive("about");
      ...
      for (var i = 0; i < 5; i++) {
        ...
        if (i < rating) {
          aboutHtmlContent = insertProperty(aboutHtmlContent, className, "fa fa-star");
        } else {
          aboutHtmlContent = insertProperty(aboutHtmlContent, className, "fa fa-star-o");
        }
      }

      document.querySelector("#main-content").innerHTML = aboutHtmlContent;
    }, false);
  };
